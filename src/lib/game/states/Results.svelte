<script lang="ts">
	...
	let showAnswersStack = true
	let showSaveMistakesButton = true
...
{#if showAnswersStack}
	<section class="flex flex-col items-center" out:slide|local={{ duration: 600 }}>
		...
		<div
			class="relative aspect-[5/7] min-w-[300px] w-[75vw] sm:w-[400px] md:w-[450px] lg:w-[500px]"
		>
...
{:else}
	<div class="text-5xl text-center my-4">Тест Завершён</div>
	...
	<section in:fade>
		<Collapse isOpen={true}>
			...
			<div class="grid gap-2 grid-cols-1 md:grid-cols-2 xl:grid-cols-3">
				...
				{:else}
					<div class="text-gray-600">Нет правильных ответов</div>
				{/each}
			</div>
		</Collapse>
		...
		<Collapse isOpen={true}>
			...
			<div class="grid gap-2 grid-cols-1 md:grid-cols-2 xl:grid-cols-3">
				...
				{:else}
					<div class="text-gray-600">Нет неправильных ответов</div>
				{/each}
			</div>
		</Collapse>
